...
function WssStatus(props) {
  ...
  useEffect(() => {
    axios.get('https://avail-tools.brightlystake.com/api/avail/wss-status')
      .then(res => {
      ...
      <div>
        ...
        <div className="network-buttons">
          {networks.map(network => (
            ...
            >
              {(network === 'Avail Turing Network') ? 'Testnet' : (network === 'Avail DA Mainnet') ? 'Mainnet' : "Failed Endpoints"}
            </button>
          ))}
        </div>
